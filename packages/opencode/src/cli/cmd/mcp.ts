...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      outro("Add servers with: opencode mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      log.info("Remote MCP servers support OAuth by default. Add a remote server in opencode.json:")
      log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
    ...
    if (authStatus === "authenticated") {
      const confirmed = yield* Effect.promise(() => confirm({
    ...
    yield* MCP.Service.use((mcp) =>
      mcp.authenticate(serverName, (url) => {
        s.stop("Authorize in your browser:")
        log.info(url)
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "remote") {
        ...
        if (useOAuth) {
          const hasClientId = await confirm({
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      try {
        ...
        s.stop(`HTTP response: ${response.status} ${response.statusText}`)
        ...
        if (response.status === 401) {
          log.info("Initial unauthenticated check returned 401, so this server requires OAuth")
